...
- Eliminar el directorio `target`; de todas formas está a punto de volver a compilar todo
- Cambiar el número de versión en `.github/workflows/main.yml`
- Cambiar el número de versión en `rust-toolchain`, que debería cambiar la
  versión que estás usando localmente con `rustup`
- Cambiar el número de versión en `src/title-page.md`
- Ejecutar `./Tools/update-rustc.sh` (consulte su código comentado para detalles 
  sobre lo que hace)
- Inspeccionar los cambios (mirando los archivos cambiados según git) y
  sus efectos (mirando los archivos en `tmp/book-before` y
        `tmp/book-after`) y `commit` si se ven correctos
- Grep para `manual-regeneration` y siga las instrucciones en esos lugares para
  actualizar la salida que no puede ser generada por un script
